// run with cargo run --bin bluetooth_le_controller//! A somewhat comprehensive example of a typical combinatoric `bpaf` usage.
...
fn main() {
    let opts = opts().run();
    println!("{:#?}", opts);
    println!("Hello, bluetooth_le_controller!");
}
